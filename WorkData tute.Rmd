...
author: "Joshua F. Wiley - last updated Michelle Byrne 7 Mar 2021"
date: "`r Sys.Date()`"
output:
  tufte::tufte_html: 
    toc: true
...
Have downloaded the `R`markdown file for the lecture by pulling the most recent git repo
[https://github.com/michellebyrne1/MonashHonoursStatistics/edit/master/WorkData.rmd](https://github.com/michellebyrne1/MonashHonoursStatistics/edit/master/WorkData.rmd)
...
d3 <- d[!is.na(PosAff) [, Count := .N, by = UserID][Count >= 20] [PosAff 33]]

d3 <- d[!is.na(PosAff)][PosAff >= 3][, 
        Count := .N, by = UserID][Count 20]
...
d3 <- d[weekday(SurveyDay) %in% cd2 <-c("Monday")]
...
## Merging by Multiple IDs - You Try It
...
daily dataset, `d`, using a *full outer join*. 
The resulting dataset, `d2` should have 6,602 rows.
